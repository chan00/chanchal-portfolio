...
const Scene = () => {
  ...
  useEffect(() => {
    // Guard against double-mount (StrictMode or HMR)
    if (initializedRef.current || !canvasDiv.current) return;
    initializedRef.current = true;

    // Clear any stale canvases left over from HMR or prior mount
    ...
    const onResize = () => {
      if (characterObj && canvasDiv.current) {
        handleResize(renderer, camera, canvasDiv as React.RefObject<HTMLDivElement>, characterObj);
      }
    };
    ...
    return () => {
      ...
      if (canvasDiv.current) {
        const canvases = canvasDiv.current.querySelectorAll("canvas");
